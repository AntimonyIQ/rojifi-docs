...
export default function DocsPage() {
  const { version, tab, slug } = useParams<{
    version?: string;
    tab?: string;
    slug?: string;
  }>();
  ...
  // 1. Resolve Version
  const activeVersionStr =
    version || DOCS_DATA.find((d) => d.isLatest)?.version || "v1";
    ...
    return (
      <div className="p-10 text-center">
        ...
        <p className="text-slate-500">
          Could not find docs for {activeTab}/{currentSlug}
        </p>
      </div>
    );
  }
          ...
          switch (section.type) {
            ...
            case "code":
              return section.codeSnippet ? (
                <DocsCode key={idx} snippet={section.codeSnippet} />
              ) : null;
            case "enum":
              return section.enumData ? (
                <DocsEnum key={idx} data={section.enumData} />
              ) : null;
            case "alert":
              return (
                <DocsAlert key={idx} variant={section.variant}>
                  {section.content}
                </DocsAlert>
              );
            case "endpoint":
            ...
            case "parameter":
              return section.parameterData ? (
                <DocsParameterTable
                  key={idx}
                  title={section.content} // Optional title (e.g. "Headers")
                  parameters={section.parameterData}
                />
              ) : null;
            case "schema":
              return section.schemaData ? (
                <DocsSchemaTable
                  key={idx}
                  title={section.content}
                  fields={section.schemaData}
                />
              ) : null;
            case "response":
              return section.responseData ? (
                <DocsResponse key={idx} data={section.responseData} />
              ) : null;
            default:
              return null;
          }
